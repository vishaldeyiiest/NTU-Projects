...
public class MainActivity extends Activity implements TextureView.SurfaceTextureListener, View.OnClickListener{
    ...
    protected DJICodecManager mCodec = null;
    private static final String TAG = MainActivity.class.getName();
    protected VideoFeeder.VideoDataCallback mReceivedVideo = null;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        initUI();
        //callback for live video view
        mReceivedVideo = new VideoFeeder.VideoDataCallback(){
            @Override
            public void onReceive(byte[] bytes, int i) {
                if(mCodec != null){
                    mCodec.sendDataToDecoder(bytes, i);
                }
            }
        };
        Camera camera = playbackDemo.getCameraInstance();
        if (camera != null) {
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        Log.e(TAG, "on Surface texture Available");
        if(mCodec != null)
            mCodec = new DJICodecManager(this, surface, width, height);
    }

    @Override
    public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width, int height) {
        Log.e(TAG, "on Surface TextureSize Changed");
    }

    @Override
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
        Log.e(TAG, "on Surface Texture Destroyed");
        if(mCodec != null) {
            mCodec.cleanSurface();
            mCodec = null;
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_download_btn:{
                ...
                File destDir = new File(Environment.getExternalStorageDirectory().getPath() + "/DJI_PlaybackDemo/");
                if (mPlaybackState.getPlaybackMode().equals(SettingsDefinitions.PlaybackMode.MULTIPLE_FILES_EDIT)) {
    ...
    {
        final Camera camera = playbackDemo.getCameraInstance();
        if(camera != null){
    ...
    private void switchCameraMode(SettingsDefinitions.CameraMode cameraMode){
        ...
        if(camera != null){
            camera.setMode(cameraMode, new CommonCallbacks.CompletionCallback() {
                ...
                public void onResult(DJIError djiError) {
                    if(djiError == null)
                        showToast("Switch camera mode success");
                    else
                        showToast(djiError.getDescription());
                }
            });
    ...
    private void initPreviewer() {
        BaseProduct product = playbackDemo.getProductInstance();
        if (product == null || !product.isConnected()) {
            ...
            if (!product.getModel().equals(Model.UNKNOWN_AIRCRAFT)) {
                if (VideoFeeder.getInstance().getVideoFeeds() != null
                    ...
                    VideoFeeder.getInstance().getVideoFeeds().get(0).setCallback(mReceivedVideo);
                }
            }
        }
    }
    ...
    protected void initCameraCallBacks() {
        if (playbackDemo.isPlaybackAvailable()){
